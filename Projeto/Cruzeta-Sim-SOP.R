...
#2.3) Simulação do balaço hídrico ###
...
S_tf <- 0
A_tf <- 0
A_med1 <- A_ti              #Estimativa inicial da area media
A_med2 <- 0
D_t <- 0
D_total <- 0
Y_t <- 0
...
for(t_meses in 841:1200)
{
  ...
  #Suprimento determinado pela regra de operacao padrao
  WA <- S_ti - S_min
  if (WA > 0)
  {
    if (WA >= D_t){
  ...
  #Soma de suprimento por periodo do ano
...
Results <- matrix(c("Confiabilidade(%)","Eficiencia(%)","Eficiencia.seco(%)", "Eficiencia.umido(%)",
                    ...
                    "Maximo de meses consecutivos sem atendimento",
...
write.table(Y.matriz, file='SOP-Ymatriz.csv', sep=';', dec=',', row.names=FALSE)
write.table(S.matriz, file='SOP-Smatriz.csv', sep=';', dec=',', row.names=FALSE)
